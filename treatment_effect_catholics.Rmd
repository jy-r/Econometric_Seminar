...
```{r, echo=FALSE}
grid.arrange(
  ggplot(prs_df[prs_df$catholic==0,])+geom_histogram(aes(x=pr_score))+ggtitle(""),
  ggplot(prs_df[prs_df$catholic==1,])+geom_histogram(aes(x=pr_score))+
...
grid.arrange(
  ...
  ggplot(dta.m) + geom_boxplot(aes(as.factor(catholic),w3income)),
  ncol=2
  )

grid.arrange(
  ggplot(dta.m.m[dta.m.m$T==0,]) +
    geom_bar(aes(x=factor(1), fill=as.factor(race_black))) + 
    coord_polar(theta = "y") + 
    guides(fill=guide_legend(title="BLACK")),
